...
void UWarriorGameplayAbility::OnGiveAbility(const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilitySpec& Spec)
{
	...
	if (AbilityActivationPolicy == EWarriorAbilityActivationPolicy::OnGiven)
	{
		...
		{
			bool bIsSuccess = ActorInfo->AbilitySystemComponent->TryActivateAbility(Spec.Handle);
		}
	}
}
...
{
	FActiveGameplayEffectHandle ActiveGameplayEffectHandle = NativeApplyEffectSpecHandleToTarget(TargetActor, 
		InSpecHandle);
	
	OutSuccessType = ActiveGameplayEffectHandle.WasSuccessfullyApplied() ? EWarriorSuccessType::Successful : 
